...
**本轮一口气完成**下面四件事(按顺序;每做完一件:验证 → 追加写 `.ralph/PROGRESS.md` → 单独 `git commit`):

1. **规划**:用 `todowrite` 工具把这四件事登记成待办清单。
2. **写码并运行**:写 `greet.js`,接受一个名字参数,打印 `Hello, <名字>!`;用 bash 运行 `node greet.js Ralph` 验证输出。
...
- [ ] `greet.js` 存在,`node greet.js Ralph` 输出 `Hello, Ralph!`
