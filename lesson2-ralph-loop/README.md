...
先拿自带案例 `smoke` 练手(`examples/smoke/`,约 2 分钟)。它是个「观测演示」:
规划(todowrite)→ 写码运行(write/bash)→ `task` 派 subagent 统计 → `skill` 生成横幅,
worker 一轮做完、小步提交(案例自带专用 worker 提示词;默认 worker 是一轮一步),
跑完的会话导进 cannbot 后多 turn、多工具、子代理轨迹都有得看:

```bash
./ralph.sh init   ~/ralph-smoke
./ralph.sh sample smoke ~/ralph-smoke     # 灌入案例目标 + demo-banner 技能 + stats-collector 子代理
