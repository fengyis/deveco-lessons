...
- **node 版本**:cannbot 的原生依赖 better-sqlite3(11.10.0)只对 node
  18/20/22/23 提供预编译包,24+(如 26)会回退现场编译、一般编不过。
  `setup.sh` 优先用当前受支持的 node,不受支持才切 nvm 的 20;
  `observe.sh` 先试当前 node 能否加载,不行再切。手动在 vendor 里
  `npm install` 时确保 node 是 20/22。
