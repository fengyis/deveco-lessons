...
| B:loop | **816/816,reviewer 验收 DONE** | ~24.5 min,2 轮 | 第 1 轮按节奏逐块推进(中途主动解剖 `wordsep_re` 正则),轮末 815/816;reviewer 跑隐藏 QA 逮住最后 1 条 → `CONTINUE` 点出根因与修法 → 第 2 轮一个 commit 修复 → 复核 DONE |

Reviewer 首轮裁决原文(plugin.log,这就是外循环喂给 worker 的东西):

```
CONTINUE: wrap 返回空向量：`wrap_chunks` 第一行被空白 chunk 填满后被 `drop_whitespace`
清空，`cur_line.is_empty()` 触发了错误 `break` 而非 `continue`，需改为
`if remaining.is_empty() { break; } continue;` 以对齐 CPython 行为。
```

worker 的下一个 commit:`fix: drop_whitespace with narrow width no longer drops all
remaining chunks` —— 反馈进,修复出,两分钟后复核通过。

注意一个诚实的细节:B 组第 1 轮(815)本身就比 A 组(795)高——同一个模型,
「一次只推进一件事」的节奏合同就是比「一次机会梭哈」做得仔细,这是 run 间方差 + 节奏
差异的叠加。loop 的**结构性**优势在于:不管第 1 轮剩 21 条还是 1 条尾巴,reviewer
都会把它逮出来并给出可执行的反馈;once 剩多少条,就永远不知道地带着多少条收工。
