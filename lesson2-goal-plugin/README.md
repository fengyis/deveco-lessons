...
4. **上游测试套件锁定移植正确性**——上游的 263 条测试原样搬过来跑,一处没改就是最强的回归保证:
...
最终到 278 pass(263 上游 + 15 本课新增,其中含坑 #8 修复的 3 条回归)。这一步的意义是:后续所有
"这处要不要改"的争论,都能用"改了会不会让这条上游测试变红"
来裁决,而不是靠直觉。
...
8. **`/goal <目标>` 提交后会被自己 6ms 内 pause 掉——命令消息豁免在 deveco 上失效。** 现象:E2E 实测
   `/goal <目标>` 提交后 `state.json` 的历史记录里 `set` 和 `paused`(`user intervention`)前后仅
   相差 6ms,目标常驻 `stopped:true`,靠 `session.idle` 驱动的自动续推永远不会触发,核心机制形同虚设。
   根因:上游 `chat.message` 靠 `text.startsWith("/goal ")` 识别"这条落盘的用户消息是命令本身触发的
   回显,不是真人打断",但 deveco 按 `$ARGUMENTS` 模板把命令展开成落盘消息时**不带 `/goal` 前缀**
   (探针实测:`/probe hello` 在 deveco 的 sqlite 里落盘就是 `"hello"`,见
   [`docs/probe-notes.md`](docs/probe-notes.md) 问题 1),前缀检查从没命中过。本仓库已修:
   `command.execute.before` 把这次命令的展开文本记进 per-session 的 `pendingCommandTexts`,
   `chat.message` 在保留的前缀检查之后追加一次性同文本豁免——命中就 `delete` 掉这条记录并放行,
...
```bash
node --test test/*.test.js   # 278 条:上游原有 + 本课新增的适配/goal.sh/smoke 场景（含坑 #8 的 3 条回归用例）
